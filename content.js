...
window.onload = async function() {
  ...
  const highlightPage = async () => {
          ...
          for (const object in biases) {
            ...
            const bias =biases[object]
                for(const argument in bias){
                  ...
                  highlightInstance(excerpt, explanation,object)
            ...
            chrome.storage.local.set({"cumulative_biases": cumulative_biases.cumulative_biases});
        })
    ...
    async function categorizeSentences(longEntries) {
              ...
              "properties": {
                ...
                "explanation": {
                  "type": "string"
                    ...
                    required: ["stereotypes", "generalizations", "unfair_statements"],
        ...
        let a = "" //placeholder to keep previous prompt incase you for some reason want those categories?
        const messages = [
            {
                role: 'user',
                content: `system_content = "You are a social bias detection tool. Analyze the following text and identify how the narrative contains or perpetuates social biases. You will return your response as a JSON object. Return any text fragments that include the following biases in the narrative: 
                ${//racial_bias, gender_bias, age_bias, religious_bias, class_bias, disability_bias, sexual_orientation_bias, cultural_bias, body_size_bias, marginalism_ethnocentrism, intersectional_bias, also look for
                a
                }
                'generalizations', 'unfair_statements', and 'stereotypes'. Return the EXACT TEXT FRAGMENTS that contain these aspects along with an analysis (IN THE SAME LANGUAGE AS THE CONTENT) of how that fragment relates to that particular bias.":\n\n${longEntries.join('\n')}`
            }
        ];
    ...
    const highlightInstance = (string,tooltip,type) => {
      //Had to use classes for the coloring because attributes cant be used for colors?
        const colorClass = colorMap[type].className;
        console.log("Highlight Instance")
        console.log("color: ", colorClass, "text: ", string, "explanation: ", tooltip)
        //What to label the description as
        const name = colorMap[type].name;

        //should create one instance and reuse, this was just faster for the hackathon
        const instance = new Mark(document.body);
              instance.mark(string, {
              className: `highlight ${colorClass}`, //coloring
              separateWordSearch: false, 
              "acrossElements": true, //KEY*, lets you highlight even if there is a bold, or a <strong> in between
              "ignoreJoiners": true,
              "wildcards": "withSpaces",
              each: (node) => {
